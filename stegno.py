...
def showimage():
  ...
  # Ensure a file is selected
    ...
    img = img.resize((250, 250))  # Resize image for better fit
...
def Hide():
    global secret
    message = text1.get("1.0", END).strip()  # Remove extra spaces/newlines
    if not filename:
...
def save():
    if 'secret' in globals():  # Check if secret exists
